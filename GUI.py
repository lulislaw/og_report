...
from main import make_main_presentation
from daily_report import make_daily_presentation
...
def generate_report_halfdaily():
    ...
    def report_task():
        ...
        if not summer:
            edc_file = None
        report_lines = make_main_presentation(ais_file, edc_file, previous_period_data, date_str, morning, config)
...
def generate_report_daily():
    ...
    def report_task():
        ...
        previous_period_data = {district: int(entry_vars[district].get() or 0) for district in entry_vars}
        config = False
        report_lines = make_daily_presentation(ais_file, previous_period_data, date_str, config)
        for line in report_lines:
            print(line)
        open_folder_button.pack(pady=10)
        print("Конец" + "\n" * 5)
...
def generate_report_weekly():
    ...
    def report_task():
        ...
        date_str = date_entry.get_date().strftime("%d.%m.%Y")
        config = False
        report_lines = make_weekly_report(ais_file, date_str, config)
        for line in report_lines:
            print(line)
        open_folder_button.pack(pady=10)
        print("Конец" + "\n" * 5)
...
def generate_report_month():
    ...
    def report_task():
        ...
        date_str = date_entry.get_date().strftime("%d.%m.%Y")
        config = False
        report_lines = make_month_report(ais_file, date_str, config)
        for line in report_lines:
            print(line)
        open_folder_button.pack(pady=10)
        print("Конец" + "\n" * 5)
...
def update_buttons_visibility():
    ...
    all_buttons = [
        halfdaily_btn, daily_btn, weekly_btn, month_btn
    ]
    for btn in all_buttons:
        btn.pack_forget()
    if report_type == "Полусуточный":
        halfdaily_btn.pack(side="right", padx=20)
        edc_file_frame.pack(pady=5)
        morning_report_frame.pack()
    elif report_type == "Суточный":
        daily_btn.pack(side="right", padx=20)
        edc_file_frame.pack_forget()
...
def on_tree_click(event):
    """
    Отслеживает клик в ячейке Treeview.
    Если пользователь кликнул по столбцу с чекбоксом, переключаем состояние.
    """
...
def load_ais_data():
    """
    Считывает ais_file, берёт только те строки, у которых «Адрес объекта»
    содержит одно из заданных слов (без учёта регистра),
    убирает дубликаты по адресу, исключает адреса из "adresses.xlsx" и выводит в Treeview.
    """

    ais_path = ais_file_entry.get()
    addresses_file = "adresses.xlsx"  # Файл с исключаемыми адресами
    ...
    try:
        ...
        # Читаем "adresses.xlsx", если файл существует
        ...
        # Очищаем Treeview и состояние чекбоксов
...
def save_tree_to_excel(only_checked=False):
    """
    Собирает данные из Treeview и сохраняет в Excel-файл.
    Если only_checked=True, то сохраняем только те строки, у которых чекбокс установлен.
    Иначе сохраняем все строки.
    При этом берём ОРИГИНАЛЬНЫЕ данные (из original_data), без переноса.
    Если файл уже существует, добавляем новые записи без дубликатов.
    """
    data = []
    for row_id in tree.get_children():
        # Если включён режим only_checked, пропускаем неотмеченные
...
edc_file_frame = ctk.CTkFrame(file_selection_frame)
edc_file_frame.pack(pady=5)

edc_file_label = ctk.CTkLabel(edc_file_frame, text="Файл ЕДЦ:", font=("Arial", 12))
edc_file_label.pack(side="left", padx=5)

edc_file_entry = ctk.CTkEntry(edc_file_frame, width=310)
edc_file_entry.pack(side="left", padx=5)
edc_file_entry.drop_target_register(DND_FILES)
edc_file_entry.dnd_bind("<<Drop>>", lambda e: drop_file(e, edc_file_entry))

edc_file_button = ctk.CTkButton(edc_file_frame, text="...", width=30, command=lambda: select_file(edc_file_entry))
edc_file_button.pack(side="left", padx=5)
...
# config_report_frame = ctk.CTkFrame(date_frame)
# config_report_frame.pack(pady=5, padx=15)
morning_var = ctk.BooleanVar(value=True)
...
# config_var = ctk.BooleanVar(value=True)
# config_checkbox = ctk.CTkCheckBox(config_report_frame, text="Конфиг", variable=config_var)
morning_checkbox.pack()
summer_checkbox.pack()
# config_checkbox.pack()
...
daily_btn = ctk.CTkButton(button_frame, text="Суточный отчет", command=generate_report_daily)
daily_btn.pack(side="right", padx=20)
...
# Горизонтальный/вертикальный скролл при необходимости
tree_scrollbar = ttk.Scrollbar(table_frame, orient="vertical")
tree_scrollbar.pack(side="right", fill="y")

# Обозначаем 4 столбца: 1) check 2) Адрес 3) Район 4) Округ
...
tree.column("check", width=30, anchor="center")  # колонка под чекбоксы
...
# Привязываем событие клика, чтобы переключать чекбоксы
tree.bind("<Button-1>", on_tree_click)

# Инициируем состояние кнопок / режимов
