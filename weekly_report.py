...
from str_pptx import keys_weekly_widget, keys_main, keys_table_weekly
from xlsx_functions import update_ais_data


def better_or(value):
    numeric_value = int(value.replace(" ", "").replace(u"\xa0", ""))
    allper_value = value
    if numeric_value > 0:
        allper_value = f"↗ {value}"
...
def make_weekly_report(ais_file, date,config, type="Недельный"):
    dist_path = ""
    date_text = f"{date}"
    ...
    allert.extend(calculation(df_ais, report_path, date))
    return allert


def calculation(df_ais, report_path, date):
    allert = []
